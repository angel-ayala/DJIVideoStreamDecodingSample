...
*  */
package com.dji.videostreamdecodingsample.WebRTC;
...
public class DJIVideoCapturer implements VideoCapturer {
    private final static String TAG = "DJIVideoCapturer";
    ...
    private void setupVideoListener(){
            ...
            public void onYuvDataReceived(MediaFormat mediaFormat, ByteBuffer videoBuffer, int dataSize, int width, int height) {
                if (videoBuffer != null){
                    try{
                        ...
                        Log.e(TAG, "onYuvDataReceived: " + e.getClass().getName() + " occurred, stack trace: " + e.getMessage() + "\n" + e.getCause());
                    }
                }
            }
        });
        ...
        Log.d(TAG, "DJICapture for : " + aircraftModel.getDisplayName());
        if (aircraftModel.getDisplayName().equals("DJI Air 2S")){
            Log.d(TAG, "DJICapture in");
